...
// .wrangler/tmp/bundle-uHOKxa/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/pages-IVD6YN/functionsWorker-0.21394770660196216.mjs
...
var convertChangeData = /* @__PURE__ */ __name2((columns, record2, options = {}) => {
  var _a;
...
var convertColumn = /* @__PURE__ */ __name2((columnName, columns, record2, skipTypes) => {
...
var RealtimePostgresFilterBuilder = /* @__PURE__ */ __name(class {
  constructor() {
    this.filters = [];
  }
...
__name2(RealtimePostgresFilterBuilder, "RealtimePostgresFilterBuilder");
...
var AuthRefreshDiscardedError = /* @__PURE__ */ __name(class extends CustomAuthError {
...
__name2(AuthRefreshDiscardedError, "AuthRefreshDiscardedError");
...
var PUBLIC_SUPABASE_URL = "https://qqmfxbcroxunvtgxxray.supabase.co";
var PUBLIC_SUPABASE_ANON_KEY = "sb_publishable_TJuVLg5h31V-kg3gX36vUQ_wKR-Ga2_";
...
var NEVER = INVALID;
var INTENTS = ["parse_booking", "improve_day"];
var AiRequest = external_exports.discriminatedUnion("intent", [
  external_exports.object({
    intent: external_exports.literal("parse_booking"),
    tripId: external_exports.string().uuid(),
  ...
  external_exports.object({
    intent: external_exports.literal("improve_day"),
    tripId: external_exports.string().uuid(),
    day: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/, "expected YYYY-MM-DD")
  })
]);
var REFUSAL_REASONS = ["disabled", "quota", "forbidden", "unavailable"];
var AiRefusal = external_exports.object({
  ok: external_exports.literal(false),
  reason: external_exports.enum(REFUSAL_REASONS),
  /** Shown to the member as-is. Never leaks internals. */
  message: external_exports.string()
});
var AiSuccess = external_exports.object({
...
var AiResponse = external_exports.union([AiSuccess, AiRefusal]);
var QUOTA_WINDOW_HOURS = 24;
var QUOTA_PER_TRIP = 40;
var ITINERARY_CATEGORIES = [
  "flight",
  "hotel",
  "activity",
  "restaurant",
...
__name2(isCalendarDate, "isCalendarDate");
var orNull = /* @__PURE__ */ __name2((inner) => external_exports.preprocess(
  (v) => v === void 0 || typeof v === "string" && v.trim() === "" ? null : v,
  inner.nullable()
), "orNull");
var ParsedBookingResult = external_exports.object({
  title: orNull(external_exports.string().trim().min(1).max(120)),
  category: orNull(external_exports.enum(ITINERARY_CATEGORIES)),
  day: orNull(external_exports.string().regex(YMD).refine(isCalendarDate, "not a real date")),
  end_day: orNull(external_exports.string().regex(YMD).refine(isCalendarDate, "not a real date")),
  start_time: orNull(external_exports.string().regex(HM)),
  end_time: orNull(external_exports.string().regex(HM)),
  location: orNull(external_exports.string().trim().min(1).max(200))
}).refine((v) => !(v.day && v.end_day) || v.end_day >= v.day, {
  message: "end_day precedes day"
}).refine((v) => !v.end_day || !!v.day, { message: "end_day without day" }).refine((v) => !!(v.day || v.start_time || v.location), {
  message: "nothing structured was extracted"
});
var DayPickResult = external_exports.object({
  planId: external_exports.string().min(1).max(32),
  reason: external_exports.string().trim().min(1).max(400)
});
var BOOKING_JSON_SCHEMA = {
  type: "object",
  properties: {
...
function bookingParsePrompt(args) {
  const range = args.tripStart && args.tripEnd ? `The trip runs ${args.tripStart} to ${args.tripEnd}.` : args.tripStart ? `The trip starts ${args.tripStart}.` : args.tripEnd ? `The trip ends ${args.tripEnd}.` : "The trip has no dates set.";
  const user = [
    range,
...
__name(bookingParsePrompt, "bookingParsePrompt");
__name2(bookingParsePrompt, "bookingParsePrompt");
var DAY_PICK_JSON_SCHEMA = {
  type: "object",
  properties: {
...
var PREFERENCES_TOKEN_BUDGET = 300;
var PREF_OPEN = "<GROUP_PREFERENCES>";
var PREF_CLOSE = "</GROUP_PREFERENCES>";
function sanitizePreference(value) {
  return value.replace(/[<>]/g, " ").replace(/\s+/g, " ").trim();
}
__name(sanitizePreference, "sanitizePreference");
__name2(sanitizePreference, "sanitizePreference");
function renderPreferences(preferences, budgetTokens = PREFERENCES_TOKEN_BUDGET) {
  if (!preferences)
    return { block: null, dropped: [] };
  const lines = [];
  const pace = preferences.pace ? sanitizePreference(preferences.pace) : "";
  if (pace)
    lines.push({ field: "pace", text: `Pace: ${pace}` });
  const budget = preferences.budgetStyle ? sanitizePreference(preferences.budgetStyle) : "";
  if (budget)
    lines.push({ field: "budgetStyle", text: `Budget style: ${budget}` });
  const dietary = (preferences.dietary ?? []).map(sanitizePreference).filter(Boolean);
  if (dietary.length)
    lines.push({ field: "dietary", text: `Dietary needs: ${dietary.join(", ")}` });
  const interests = (preferences.interests ?? []).map(sanitizePreference).filter(Boolean);
  if (interests.length)
    lines.push({ field: "interests", text: `Interests: ${interests.join(", ")}` });
  const notes = preferences.notes ? sanitizePreference(preferences.notes) : "";
  if (notes)
    lines.push({ field: "notes", text: `Also: ${notes}` });
  if (!lines.length)
    return { block: null, dropped: [] };
  const estimate = /* @__PURE__ */ __name2((ls) => Math.ceil((PREF_OPEN.length + 1 + PREF_CLOSE.length + ls.reduce((n, l) => n + l.text.length + 1, 0)) / 4), "estimate");
  const DROP_ORDER = ["notes", "interests", "budgetStyle", "pace"];
  const kept = [...lines];
  const dropped = [];
  for (const field of DROP_ORDER) {
    if (estimate(kept) <= budgetTokens)
      break;
...
__name(renderPreferences, "renderPreferences");
__name2(renderPreferences, "renderPreferences");
function improveDayPrompt(args) {
  const km = /* @__PURE__ */ __name2((n) => `${n.toFixed(1)} km`, "km");
  const where = args.placeName ? ` in ${args.placeName}` : "";
  const lines = [
    `The day is ${args.day}${where}.`,
    `As it stands it involves about ${km(args.baseline.travelKm)} of travel` + (args.baseline.conflicts > 0 ? ` and has ${args.baseline.conflicts} overlapping ${args.baseline.conflicts === 1 ? "item" : "items"}.` : "."),
    "",
    "Plans:"
  ];
  for (const c of args.candidates) {
    lines.push(
      `${c.id}: ${c.order.join(" \u2192 ")}`,
      `  travel ${km(c.travelKm)} \xB7 ${c.moved} ${c.moved === 1 ? "item moves" : "items move"} \xB7 ends ${c.endsAt}`
    );
  }
...
__name(improveDayPrompt, "improveDayPrompt");
__name2(improveDayPrompt, "improveDayPrompt");
var MODELS = {
...
function workersAiProvider(ai) {
  return {
    async complete({ system, user, schema, maxOutputTokens, tier }) {
      ...
      let json3 = raw?.response;
      if (json3 === void 0) {
        const text = raw?.choices?.[0]?.message?.content;
        try {
          json3 = typeof text === "string" ? JSON.parse(text) : void 0;
        } catch {
          json3 = void 0;
        }
      }
...
__name2(workersAiProvider, "workersAiProvider");
var EARTH_RADIUS_KM = 6371;
var WALK_KMH = 4.8;
var DRIVE_KMH = 30;
var WALK_MAX_KM = 1.5;
var toRad = /* @__PURE__ */ __name2((deg) => deg * Math.PI / 180, "toRad");
...
__name(estimateLeg, "estimateLeg");
__name2(estimateLeg, "estimateLeg");
...
__name(toGeoPoint, "toGeoPoint");
__name2(toGeoPoint, "toGeoPoint");
var FIXED_CATEGORIES = /* @__PURE__ */ new Set(["flight", "hotel", "restaurant", "transport"]);
var DEFAULT_MINUTES = 60;
var MIN_IMPROVEMENT_KM = 0.5;
var MOVE_PENALTY_KM = 0.3;
var MAX_PERMUTED = 6;
var MAX_CANDIDATES = 3;
var LATEST_END = 22 * 60;
...
__name(toHm, "toHm");
__name2(toHm, "toHm");
function classifyDay(items, day) {
  const anchors = [];
  const movable = [];
  const excluded = [];
  for (const item of items) {
    ...
    const scheduled = { item, start, end, assumedEnd };
    if (FIXED_CATEGORIES.has(item.category))
      anchors.push(scheduled);
    else
      movable.push(scheduled);
  }
  anchors.sort((a, b) => a.start - b.start);
  movable.sort((a, b) => a.start - b.start);
  return { anchors, movable, excluded };
}
__name(classifyDay, "classifyDay");
__name2(classifyDay, "classifyDay");
function travelKm(ordered) {
  const points = ordered.map((s) => toGeoPoint({ latitude: s.item.lat, longitude: s.item.lng })).filter((p) => p != null);
  let km = 0;
  for (let i = 1; i < points.length; i++)
    km += haversineKm(points[i - 1], points[i]);
  return { km, located: points.length };
}
__name(travelKm, "travelKm");
__name2(travelKm, "travelKm");
function countConflicts(ordered) {
  let n = 0;
...
__name(countConflicts, "countConflicts");
__name2(countConflicts, "countConflicts");
...
__name(greedyTours, "greedyTours");
__name2(greedyTours, "greedyTours");
function windowEndFor(movable, anchors) {
  ...
  if (needed <= natural - start - blocked)
    return { end: natural, extended: false };
  return { end: Math.max(natural, LATEST_END), extended: true };
}
__name(windowEndFor, "windowEndFor");
__name2(windowEndFor, "windowEndFor");
function freeWindows(movable, anchors, windowEnd) {
  const windowStart = Math.min(...movable.map((s) => s.start));
  let windows = [[windowStart, windowEnd]];
...
__name(freeWindows, "freeWindows");
__name2(freeWindows, "freeWindows");
...
__name(transitMinutes, "transitMinutes");
__name2(transitMinutes, "transitMinutes");
function place(order, windows) {
  const placed = [];
  let wi = 0;
  ...
  for (const s of order) {
    ...
    while (wi < windows.length) {
      ...
      if (Math.max(from, ws) + duration <= we)
        break;
      wi++;
...
function planDay(items, day) {
  ...
  if (movable.length < 2)
    return { baseline, candidates: [], notes, excluded };
  ...
  const cost = /* @__PURE__ */ __name2((c) => c.travelKm + MOVE_PENALTY_KM * c.moved, "cost");
  const worthwhile = scored.filter((c) => c.moved > 0).filter(
    (c) => (
  ...
  ).sort((a, b) => cost(a) - cost(b)).slice(0, MAX_CANDIDATES).map((c, i) => ({ ...c, id: `plan-${i + 1}` }));
...
__name2(planDay, "planDay");
var refuse = /* @__PURE__ */ __name2((reason, message, status) => ({ status, body: { ok: false, reason, message } }), "refuse");
async function record(deps, args) {
  try {
...
async function tripRange(deps, tripId) {
  try {
    const { data, error } = await deps.db.from("trips").select("start_date, end_date").eq("id", tripId).maybeSingle();
    if (error || !data)
...
__name(tripRange, "tripRange");
__name2(tripRange, "tripRange");
async function handleAiRequest(rawBody, deps) {
  if (!deps.enabled) {
    return refuse(
      "disabled",
  ...
  try {
    const { data, error } = await deps.db.from("members").select("id").eq("trip_id", tripId).limit(1).maybeSingle();
    if (error || !data) {
  ...
  const since = new Date(now.getTime() - QUOTA_WINDOW_HOURS * 36e5).toISOString();
  let used;
  try {
    const { count, error } = await deps.db.from("ai_usage").select("id", { count: "exact", head: true }).eq("trip_id", tripId).gte("created_at", since);
    if (error || count == null) {
...
__name2(handleAiRequest, "handleAiRequest");
async function parseBooking(deps, tripId, text) {
...
__name(parseBooking, "parseBooking");
__name2(parseBooking, "parseBooking");
function computedReason(plan, plans) {
  const parts = [];
  const saved = plans.baseline.travelKm - plan.travelKm;
  if (saved >= 0.1)
    parts.push(`saves about ${saved.toFixed(1)} km of travel`);
  ...
  if (!parts.length)
    parts.push("groups nearby stops together");
  const moved = `${plan.moved} ${plan.moved === 1 ? "item moves" : "items move"}`;
  return `This order ${parts.join(" and ")}. ${moved}, finishing at ${plan.endsAt}.`;
}
__name(computedReason, "computedReason");
__name2(computedReason, "computedReason");
var nothingToDo = /* @__PURE__ */ __name2((message, plans) => ({
  status: 200,
  body: {
    ok: true,
    intent: "improve_day",
...
async function improveDay(deps, tripId, day) {
  ...
  if (plans.candidates.length === 0) {
    ...
    return nothingToDo(
      plans.baseline.total < 2 ? "There is not enough in this day to rearrange yet." : "This day already looks well ordered \u2014 nothing worth moving.",
      plans
    );
  }
  ...
  const chosen = picked.success ? plans.candidates.find((c) => c.id === picked.data.planId) : void 0;
...
__name(improveDay, "improveDay");
__name2(improveDay, "improveDay");
function suggestion(plans, plan, reason, reasonSource, usage) {
  return {
    status: 200,
    body: {
      ok: true,
...
__name(suggestion, "suggestion");
__name2(suggestion, "suggestion");
var isAiBinding = /* @__PURE__ */ __name2((v) => typeof v === "object" && v !== null && typeof v.run === "function", "isAiBinding");
...
async function onRequestPost({ request, env }) {
  ...
  const { status, body: result } = await handleAiRequest(body, {
    ...
    provider: isAiBinding(env.AI) ? workersAiProvider(env.AI) : void 0
  });
  return json(result, status);
}
__name(onRequestPost, "onRequestPost");
__name2(onRequestPost, "onRequestPost");
var EDGE_TTL_SECONDS = 24 * 60 * 60;
var json2 = /* @__PURE__ */ __name2((body, status, cacheable) => new Response(JSON.stringify(body), {
  status,
  headers: {
    "content-type": "application/json; charset=utf-8",
    "cache-control": cacheable ? `public, max-age=${EDGE_TTL_SECONDS}` : "no-store"
  }
}), "json");
var unconfigured = /* @__PURE__ */ __name2(() => json2({ ok: false, reason: "unconfigured" }, 501, false), "unconfigured");
var badRequest = /* @__PURE__ */ __name2((message) => json2({ ok: false, reason: "bad_request", message }, 400, false), "badRequest");
var upstreamFailed = /* @__PURE__ */ __name2(() => json2({ ok: false, reason: "upstream" }, 502, false), "upstreamFailed");
var ok = /* @__PURE__ */ __name2((body) => json2({ ok: true, ...body }, 200, true), "ok");
function apiKey(env) {
  if (env.GEOAPIFY_ENABLED === "false")
    return null;
  const key = env.GEOAPIFY_KEY?.trim();
  return key ? key : null;
}
__name(apiKey, "apiKey");
__name2(apiKey, "apiKey");
async function withEdgeCache(ctx, build) {
  const cacheKey = new Request(ctx.request.url, { method: "GET" });
  const hit = await caches.default.match(cacheKey).catch(() => void 0);
  if (hit)
    return hit;
...
__name(callGeoapify, "callGeoapify");
__name2(callGeoapify, "callGeoapify");
function isSameOrigin(request) {
  if (request.headers.get("sec-fetch-site") === "same-origin")
    return true;
  const self2 = new URL(request.url).host;
  const hostOf = /* @__PURE__ */ __name2((value) => {
    if (!value)
      return null;
    try {
  ...
  if (referer)
    return referer === self2;
  return false;
}
__name(isSameOrigin, "isSameOrigin");
__name2(isSameOrigin, "isSameOrigin");
var notOurs = /* @__PURE__ */ __name2(() => json2({ ok: false, reason: "forbidden" }, 403, false), "notOurs");
...
__name2(numberParam, "numberParam");
function normalizeQuery(query) {
  return query.trim().toLowerCase().replace(/\s+/g, " ");
}
__name(normalizeQuery, "normalizeQuery");
__name2(normalizeQuery, "normalizeQuery");
var PLACES_URL = "https://api.geoapify.com/v2/places";
var AUTOCOMPLETE_URL = "https://api.geoapify.com/v1/geocode/autocomplete";
var SEARCH_URL = "https://api.geoapify.com/v1/geocode/search";
...
var REQUESTED_CATEGORIES = CATEGORY_PREFIXES.map((c) => c.prefix).join(",");
function categorizeGeoapify(categories) {
  if (!Array.isArray(categories))
    return null;
  const values = categories.filter((c) => typeof c === "string");
  for (const { prefix, bucket } of CATEGORY_PREFIXES) {
    if (values.some((v) => v === prefix || v.startsWith(`${prefix}.`)))
      return bucket;
  }
  return null;
}
__name(categorizeGeoapify, "categorizeGeoapify");
__name2(categorizeGeoapify, "categorizeGeoapify");
function buildPlacesUrl(center, { radiusMeters, limit }, apiKey2) {
...
__name(buildPlacesUrl, "buildPlacesUrl");
__name2(buildPlacesUrl, "buildPlacesUrl");
...
__name(featuresOf, "featuresOf");
__name2(featuresOf, "featuresOf");
...
__name(propsOf, "propsOf");
__name2(propsOf, "propsOf");
var finite = /* @__PURE__ */ __name2((v) => typeof v === "number" && Number.isFinite(v), "finite");
function parseGeoapifyPlaces(data, limit = 24) {
  if (limit <= 0)
    return [];
  const seen = /* @__PURE__ */ new Set();
  const places = [];
  for (const feature of featuresOf(data)) {
    ...
    if (seen.has(id) || seen.has(dedupeKey))
      continue;
    seen.add(id);
    seen.add(dedupeKey);
...
__name(parseGeoapifyPlaces, "parseGeoapifyPlaces");
__name2(parseGeoapifyPlaces, "parseGeoapifyPlaces");
function buildGeocodeUrl(query, { limit, autocomplete }, apiKey2) {
  const params = new URLSearchParams({
    text: query,
    limit: String(limit),
    lang: "en",
...
__name(buildGeocodeUrl, "buildGeocodeUrl");
__name2(buildGeocodeUrl, "buildGeocodeUrl");
function formatGeoapifyLabel(props) {
  ...
  for (const part of parts) {
    if (typeof part !== "string" || !part || seen.has(part))
      continue;
    seen.add(part);
    unique.push(part);
  }
...
__name(formatGeoapifyLabel, "formatGeoapifyLabel");
__name2(formatGeoapifyLabel, "formatGeoapifyLabel");
function parseGeoapifySuggestions(data, limit = 6) {
  if (limit <= 0)
    return [];
  const seen = /* @__PURE__ */ new Set();
  const out = [];
...
__name(parseGeoapifySuggestions, "parseGeoapifySuggestions");
__name2(parseGeoapifySuggestions, "parseGeoapifySuggestions");
function parseGeoapifyFirstCoords(data) {
  for (const feature of featuresOf(data)) {
    const props = propsOf(feature);
    if (!props)
      continue;
...
__name(parseGeoapifyFirstCoords, "parseGeoapifyFirstCoords");
__name2(parseGeoapifyFirstCoords, "parseGeoapifyFirstCoords");
var MAX_QUERY = 200;
var MAX_LIMIT = 10;
async function onRequestGet(ctx) {
  ...
  if (query.length < 3)
    return badRequest("Expected a query of at least 3 characters.");
  if (query.length > MAX_QUERY)
    return badRequest("That query is too long.");
  ...
  return withEdgeCache(ctx, async () => {
    ...
    return single ? ok({ coords: parseGeoapifyFirstCoords(body) }) : ok({ suggestions: parseGeoapifySuggestions(body, limit) });
  });
}
__name(onRequestGet, "onRequestGet");
__name2(onRequestGet, "onRequestGet");
var MAX_RADIUS_M = 5e3;
var MAX_LIMIT2 = 50;
async function onRequestGet2(ctx) {
  ...
  const limit = Math.min(MAX_LIMIT2, Math.max(1, numberParam(params, "limit") ?? 24));
  return withEdgeCache(ctx, async () => {
...
// .wrangler/tmp/bundle-uHOKxa/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-uHOKxa/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.21394770660196216.js.map
